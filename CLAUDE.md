...
- **Mosul and Nineveh split site-wide (Aug 3):** `mosul` ("Mosul"/"الموصل") and `nineveh` ("Nineveh"/"نينوى") are separate cities in every filter/selector. The sample "Mosul Rising" tour stays city `mosul` (old-city content); Aknaf's UNESCO tour moved to `nineveh` (its text centers on Hatra, Nineveh governorate). Tour names may mention both.
